    ...
    ?>
        ...
        <main class="mx-4 my-3 overflow-scroll">
            <table id="example" class="table table-striped" style="width:100%;">
                ...
                <tbody>
                    <?php
                    $ret = mysqli_query($con, "SELECT DISTINCT `txn_id`,`id`,`email`,`phone`,`address`,`phone`,`address`,`name`,`paid_amount`,`modified`,`payment_status`,`deliverystatus`, GROUP_CONCAT(`orderdescription` SEPARATOR ' ') as `orderlist` FROM `orders` where `deliverystatus`='delivered' GROUP BY `txn_id` ");
                    $serial = 0;
                    while ($row = mysqli_fetch_array($ret)) {
                        $serial = $serial + 1;
                    ?>
