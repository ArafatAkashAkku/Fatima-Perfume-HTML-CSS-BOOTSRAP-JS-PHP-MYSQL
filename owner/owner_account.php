...
<body class="overflow-x-hidden">
        ...
        <main>
            <div class="d-flex flex-column align-items-center justify-content-center p-5 bg-warning">
                <div class="bg-light p-3 res-width">
                    <h2 class="text-muted text-center pt-2">Update Owner Info Details</h2>
                    <?php
                    $ret = mysqli_query($con, "select * from owner_info where email='$email' and id='$id'");
                    while ($row = mysqli_fetch_array($ret)) {
                    ?>
    ...
    if (isset($_POST['submit'])) {
        $user_exist_query = "SELECT * from `owner_info` WHERE `email`='$email' and `id`='$id'";
        $result = mysqli_query($con, $user_exist_query);
        if ($result) {
            if (mysqli_num_rows($result) > 0) {
                $query ="UPDATE `owner_info` SET `password`='$_POST[password]' WHERE `email`='$email' and `id`='$id'";
